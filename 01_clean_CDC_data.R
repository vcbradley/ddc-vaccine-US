...
# 00_clean_CDC_data.R
#
# code to clean CDC data

# working directory should be set to main repo
...
getPctVaxxedUnder18 = function(cdc_age_path){
  ...
  cdcdata_age <- cdcdata_age %>%
    group_by(date) %>%
    mutate(n_fully_vaccinated = as.numeric(n_fully_vaccinated)
    ...
    summarize(n_vax_notx = sum(n_vaccinated)
...
cleanCDCdata = function(cdc_path
  ...
  cdcdata <- left_join(cdcdata, cdc_by_age, by = 'date') %>%
    ...
    mutate(n_vax_tx = n_vaccinated_allages - n_vax_notx    # impute TX-only doses
  ...
  state_pop_totals = getStatePopTotals(filepath = statepop_filepath, download = statepop_download) %>%
    filter(state == 'US') %>% select(-state_name)
...
getOWIDdata = function(download_date = NULL
  ...
  owid = fread(filepath) %>%
    ...
    mutate(state_name = ifelse(state_name == 'New York State', 'New York', state_name)
           , date = as.Date(date)
           ) %>%
  ...
  owid <- owid %>%
    group_by(state_name) %>%
    mutate(n_pop_vaccinated = na_interpolation(people_vaccinated)
