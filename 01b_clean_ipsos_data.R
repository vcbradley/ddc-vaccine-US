...
sav_files <- dir_ls("data/raw/axios-ipsos", regexp = "sav$")
# %>%
# setdiff("data/axios_ipsos/Axios W42 032221.sav")
...
read_drop <- function(x) {
  read_sav(x) %>%
    ...
    mutate(date = str_remove_all(path_file(x), "(Axios-Ipsos\\s|Data\\.sav)"),
...
ax_raw <- map_dfr(.x = sav_files, .f = read_drop) %>%
  mutate(educ = fct_collapse(ppeduc5,
                             `HS or Less` = c("No high school diploma or GED",
...
# check
ax_coded %>% count(Q107_1, Q73, willing, hesitant, vaccinated)
...
# ax_coded %>%
#   filter(!is.na(acsnet3)) %>%  # private offline flag
